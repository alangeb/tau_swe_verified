...
`results.jsonl` is not included in this repository (large file). Run the pipeline
to regenerate it, or use `status.py` on your local results.
...
Start a vLLM server with the model. Example `docker-compose.yaml`:
...
3. **Results** — `results.jsonl` captures all outcomes (generated during runs, not included in repo). `artifacts/` stores per-instance patches, logs, and metadata.
...
# Start your model server (see Docker Compose section above)
```
...
| `artifacts/` | Per-instance patches, logs, metadata (generated) |
| `results.jsonl` | All results (generated, not included in repo) |
...
- `results.jsonl` is not included in the repo (large file, regenerated by pipeline).
